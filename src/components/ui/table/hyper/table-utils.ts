import type { FilterFn, RowData } from '@octanejs/tanstack-table'
import {
...
type FuzzyFeatures = typeof features
...
const filterFn_enhancedEquals: FilterFn<FuzzyFeatures, RowData> = (row, columnId: string, filterValue: unknown) => {
...
filterFn_enhancedEquals.resolveFilterValue = (val: unknown) => isFalsy(val)

const filterFn_enhancedGreaterThan: FilterFn<FuzzyFeatures, RowData> = (
  row,
  columnId: string,
  filterValue: unknown
) => {
...
filterFn_enhancedGreaterThan.resolveFilterValue = (val: unknown) => isFalsy(val)

const filterFn_enhancedGreaterThanOrEqualTo: FilterFn<FuzzyFeatures, RowData> = (
  row,
  columnId: string,
  filterValue: unknown
) => {
...
filterFn_enhancedGreaterThanOrEqualTo.resolveFilterValue = (val: unknown) => isFalsy(val)

const filterFn_enhancedLessThan: FilterFn<FuzzyFeatures, RowData> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue(columnId)
...
filterFn_enhancedLessThan.resolveFilterValue = (val: unknown) => isFalsy(val)

const filterFn_enhancedLessThanOrEqualTo: FilterFn<FuzzyFeatures, RowData> = (
  row,
  columnId: string,
  filterValue: unknown
) => {
...
filterFn_enhancedLessThanOrEqualTo.resolveFilterValue = (val: unknown) => isFalsy(val)

const filterFn_startsWith: FilterFn<FuzzyFeatures, RowData> = (row, columnId: string, filterValue: string) => {
  const value = String(row.getValue(columnId) ?? '').toLowerCase()
  return value.startsWith(filterValue.toLowerCase())
}

filterFn_startsWith.resolveFilterValue = (val: unknown) => isFalsy(val)

const filterFn_endsWith: FilterFn<FuzzyFeatures, RowData> = (row, columnId: string, filterValue: string) => {
  const value = String(row.getValue(columnId) ?? '').toLowerCase()
  return value.endsWith(filterValue.toLowerCase())
}

filterFn_endsWith.resolveFilterValue = (val: unknown) => isFalsy(val)

const filterFn_isEmpty: FilterFn<FuzzyFeatures, RowData> = (row, columnId: string) => {
  const value = row.getValue(columnId)
  return value === undefined || value === null || value === '' || (Array.isArray(value) && value.length === 0)
}

filterFn_isEmpty.resolveFilterValue = (val: unknown) => isFalsy(val)

const filterFn_inBetween: FilterFn<FuzzyFeatures, RowData> = (row, columnId: string, filterValue: unknown) => {
  if (Array.isArray(filterValue)) {
...
filterFn_inBetween.autoRemove = (val: unknown) => isFalsy(val)

const filterFn_isRelativeToToday: FilterFn<FuzzyFeatures, RowData> = (row, columnId: string, filterValue: unknown) => {
  const rowValue = row.getValue(columnId)
  ...
  const diffInDays = Math.floor((rowDate.getTime() - today.getTime()) / (1000 * 60 * 60 * 24))
...
filterFn_isRelativeToToday.autoRemove = (val: unknown) => isFalsy(val)

export const dynamicFilterFn: FilterFn<FuzzyFeatures, RowData> = (row, columnId: string, filterValue: unknown) => {
  let operator: FilterOperator = 'includesString'
  let value = filterValue
  let joinOperator: JoinOperator = 'and'

  const filters: Array<ExtendedColumnFilter> = row.table.store.state.columnFilters.filter(
    (f: ExtendedColumnFilter) => f.id === columnId
  )
